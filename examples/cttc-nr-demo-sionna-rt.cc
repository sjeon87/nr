...
/**
 * @ingroup examples
 * @file cttc-nr-demo-sionna-rt.cc
 * @brief A simple NR demo using the Sionna RT channel model
 *
 * Notice: this entire program uses technical terms defined by the 3GPP TS 38.300 [1].
 *
 * This example describes how to setup a simulation using the Sionna RT channel model.
 * The example consists of a simple grid topology, in which you can choose the
 * number of gNBs and UEs. Have a look at the possible parameters to know what
 * you can configure through the command line.
 *
 * With the default configuration, the example will create two flows over one
 * operational band. A second operational band can be enabled from the command
 * line, with each band containing one component carrier and one bandwidth part.
 *
 * The example will print on-screen the end-to-end result of one (or two) flows,
 * as well as writing them to a file.
 *
 * \code{.unparsed}
$ ./ns3 run "cttc-nr-demo-sionna-rt --PrintHelp"
    \endcode
 *
 */
...
main(int argc, char* argv[])
{
    ...
    RtPathSolverConfig.maxDepth = 1;               // Maximum reflection/refraction depth
    RtPathSolverConfig.los = true;                 // Include line-of-sight path
    RtPathSolverConfig.specularReflection = false; // Enable specular reflections
    ...
    RtPathSolverConfig.edgeDiffraction = false;    // Disable edge diffraction
    RtPathSolverConfig.refraction = false;         // Enable refractions
    RtPathSolverConfig.syntheticArray = false;     // Use synthetic array processing
    RtPathSolverConfig.seed = 49;                  // Random seed
    /*
    ...
    cmd.AddValue("edgeDiffraction", "Enable edge diffraction", RtPathSolverConfig.edgeDiffraction);
    ...
    /* Assign default values for sionna RT channel model*/
    Config::SetDefault("ns3::SionnaRtChannelModel::UpdatePeriod",
                       TimeValue(MilliSeconds(50))); // update the channel at each iteration
    Config::SetDefault("ns3::SionnaRtChannelModel::Scenario", StringValue(Scenario));
    ...
    /**
     * The channel is configured by this helper using a combination of the scenario, and the fading
     * channel model.
     */
    ...
    {
        channelHelper->AssignChannelsToBands({band1, band2}, NrChannelHelper::INIT_FADING);
    ...
    nrHelper->AssignStreams(ueVoiceNetDev, randomStream);
    /*
