...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("schedulerType", "Scheduler algorithm: PF, RR, Qos, or Ai", schedulerType);
    cmd.AddValue("enableLcLevelQos", "Enable QoS-aware LC scheduler", enableQoSLcScheduler);
    cmd.Parse(argc, argv);
    ...
    Ptr<NrMacSchedulerAiMsgEnv> msgEnv = CreateObject<NrMacSchedulerAiMsgEnv>(totalFlows);
    if (schedulerType == "Ai")
    {
        nrHelper->SetSchedulerAttribute(
            "NotifyCbDl",
            CallbackValue(MakeCallback(&NrMacSchedulerAiMsgEnv::NotifyCurrentIteration, msgEnv)));
    ...
    for (const auto& [flowId, flowStats] : stats)
    {
        ...
        outFile << "  TxOffered:  " << flowStats.txBytes * 8.0 / flowDuration / 1e6 << " Mbps\n";
        outFile << "  Rx Bytes:   " << flowStats.rxBytes << "\n";
        if (flowStats.rxPackets > 0)
        {
            double tput = flowStats.rxBytes * 8.0 / flowDuration / 1e6;
            double delay = 1000 * flowStats.delaySum.GetSeconds() / flowStats.rxPackets;
            double jitter = 1000 * flowStats.jitterSum.GetSeconds() / flowStats.rxPackets;
