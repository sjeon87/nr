 ...
 * reproduced exactly so results are directly comparable, with one change:
 ...
 * with Non-GBR traffic, and odd UEs receive two flows with Non-GBR and Delay
 * Critical GBR traffic.
 *
 ...
 * and the priority traffic scenario (saturation or medium-load).
 *
...
#include "ns3/nr-mac-scheduler-ai-ns3-msg-interface-env.h"
#include "ns3/nr-module.h"
#include "ns3/point-to-point-module.h"

using namespace ns3;

NS_LOG_COMPONENT_DEFINE("NrAiSched");
...
main(int argc, char* argv[])
{
    ...
    if (schedulerType == "Ai")
    {
        aiEnv = CreateObject<NrMacSchedulerAiNs3MsgInterfaceEnv>();
        nrHelper->SetSchedulerAttribute(
            "NotifyCbDlMsg",
            CallbackValue(
                MakeCallback(&NrMacSchedulerAiNs3MsgInterfaceEnv::NotifyCurrentIteration, aiEnv)));
        nrHelper->SetSchedulerAttribute(
            "ActiveDlAi",
            BooleanValue(true)); // Activate the AI model for the downlink
        std::cout << "ns3-ai message-interface AI scheduler enabled (DL)" << std::endl;
    }
#else
    NS_ASSERT_MSG(schedulerType != "Ai",
                  "The ns3-ai (ai) module is not enabled. Please enable it to use the AI "
                  "scheduler");
#endif
    ...
    */
    ...
    if (aiEnv)
    {
        // Tell the Python agent the episode is over (native ns3-ai finish flag).
        aiEnv->NotifySimulationEnd();
    }
#endif
    Simulator::Destroy();
    return 0;
