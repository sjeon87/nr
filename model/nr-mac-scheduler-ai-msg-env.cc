...
NrMacSchedulerAiMsgEnv::NrMacSchedulerAiMsgEnv() {
  ...
  interface->SetMemorySize(sizeof(NrSchedEnvMsg) + sizeof(NrSchedActMsg) +
                           4096);
  interface->SetNames("NrAiSchedSeg", "NrAiSchedEnv", "NrAiSchedAct",
                      "NrAiSchedLock");
...
NrMacSchedulerAiMsgEnv::~NrMacSchedulerAiMsgEnv() { NS_LOG_FUNCTION(this); }
TypeId NrMacSchedulerAiMsgEnv::GetTypeId() {
...
void NrMacSchedulerAiMsgEnv::DoDispose() {
  NS_LOG_FUNCTION(this);
...
void NrMacSchedulerAiMsgEnv::NotifyCurrentIteration(
  ...
  envMsg->numFlows =
      std::min(static_cast<uint32_t>(observations.size()), MAX_FLOWS);
