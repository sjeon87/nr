...
public:
   ...
   */
  NrMacSchedulerUeInfoAi(float alpha, uint16_t rnti, BeamId beamId,
   ...
   */
  typedef Callback<void, const std::vector<LcObservation> &, bool, float,
                   const std::string &, const UpdateAllUeWeightsFn &>
      NotifyCb;
   ...
   */
  static bool
  CompareUeWeightsDl(const NrMacSchedulerNs3::UePtrAndBufferReq &lue,
   ...
   */
  static double
  CalculateDlWeight(const NrMacSchedulerNs3::UePtrAndBufferReq &ue) {
    double weight = 0;
    ...
    for (const auto &ueLcg : ue.first->m_dlLCG) {
      ...
      for (const auto lcId : ueActiveLCs) {
        ...
        NS_ASSERT_MSG(it != uePtr->m_weightsDl.end(),
   ...
   */
  static bool
  CompareUeWeightsUl(const NrMacSchedulerNs3::UePtrAndBufferReq &lue,
                     const NrMacSchedulerNs3::UePtrAndBufferReq &rue) {
   ...
   */
  static double
  CalculateUlWeight(const NrMacSchedulerNs3::UePtrAndBufferReq &ue) {
    double weight = 0;
    auto uePtr = dynamic_cast<NrMacSchedulerUeInfoAi *>(ue.first.get());
    ...
    for (const auto &ueLcg : ue.first->m_ulLCG) {
      ...
      for (const auto lcId : ueActiveLCs) {
        ...
        NS_ASSERT_MSG(it != uePtr->m_weightsUl.end(),
                      "Weight not found for LC " << lcId);
        weight += it->second;
      }
    }
  ...
  Weights
      m_weightsDl; //!< Weights assigned to each flow for a UE in the downlink
