...
public:
  NrTestExtendedObservationCase() : TestCase("NrTestExtendedObservationCase") {}

private:
  void DoRun() override;
};
void NrTestExtendedObservationCase::DoRun() {
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL(obs[0].potentialTput, 8000.0f, 0.01f,
                            "potentialTput mismatch");
}
/**
...
public:
  NrTestLyapunovRewardCase() : TestCase("NrTestLyapunovRewardCase") {}

private:
  void DoRun() override;
};
void NrTestLyapunovRewardCase::DoRun() {
  NS_LOG_FUNCTION(this);
  auto getRbPerRbg = []() -> uint32_t { return 1; };
  // --- eMBB UE (5QI = 9) ---
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL(embbReward, expectedEmbb, 0.01f,
                            "eMBB Lyapunov reward mismatch");
  ...
  NS_TEST_ASSERT_MSG_EQ_TOL(urllcReward, expectedUrllc, 0.01f,
                            "URLLC Lyapunov reward mismatch");
  // URLLC with no violation (delay < PDB)
  NrMacSchedulerUeInfoAi ueUrllcOk(0.0, /*rnti=*/3, BeamId(0, 0.0),
                                   getRbPerRbg);
  {
    auto lcg = std::make_unique<NrMacSchedulerLCG>(0);
    nr::LogicalChannelConfigListElement_s lcConf;
